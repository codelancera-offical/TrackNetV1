...
import shutil    # 新增: 导入 shutil 模块

"""
python infer_on_video.py --model_path /app/models/model.pth --video_path /app/videos/example/examplexxx.mp4 --video_out_path /app/videos/output/examplexxx.mp4 --extrapolation
...
def infer_model(frames, model, device):
    ...
    for num in tqdm(range(2, len(frames)), desc="Model inference"):
        ...
        x_pred, y_pred = postprocess(output, scale=3)
...
def write_track(frames, ball_track, temp_frame_dir, trace=7):
    """ 将带有轨迹的帧保存为图片序列
    """
    ...
    pbar = tqdm(total=len(frames), desc="Saving frames with track")
    for num in range(len(frames)):
        frame = frames[num].copy()  # 使用 copy() 以免修改原始 frames
        for i in range(trace):
            if (num-i > 0):
                if ball_track[num-i][0]:
                    ...
                    break
        cv2.imwrite(os.path.join(temp_frame_dir, f"{num:06d}.png"), frame)
        pbar.update(1)
    pbar.close()
    end_write_time = time.time()
    print(f"写入图片序列总耗时: {end_write_time - start_write_time:.2f} 秒")
...
if __name__ == '__main__':
    ...
    temp_frame_dir = '/app/temp_frames'
    temp_frame_dir = write_track(frames, ball_track, temp_frame_dir, fps)

    try:
        print("\n开始使用 ffmpeg 软件编码...")
        # 核心修改：使用 libopenh264 编码器，并移除 -preset 参数
        ffmpeg_command = [
            'ffmpeg',
            '-y',
            '-framerate', str(fps),
            '-i', os.path.join(temp_frame_dir, "%06d.png"),
            '-c:v', 'libopenh264', # 使用软件编码器
            '-pix_fmt', 'yuv420p',
            args.video_out_path
        ]
