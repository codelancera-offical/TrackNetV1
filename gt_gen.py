...
"""
脚本功能: 一个完整的数据准备工具，它将你下载的原始数据集 (Dataset 目录) 转换成 datasets.py 所需的格式
使用方法: python gt_gen.py --path_input /app/Dataset --path_output /app/datasets/trackNet
"""
...
def create_gt_labels(path_input, path_output, train_rate=0.7):
    ...
    for game_id in range(1,11):
        game = 'game{}'.format(game_id)
        game_path = os.path.join(path_input, game)
        if not os.path.exists(game_path):
            print(f"Warning: Directory {game_path} not found. Skipping.")
            continue
            
        clips = os.listdir(game_path)
        for clip in clips:
            labels_path = os.path.join(path_input, game, clip, 'Label.csv')
            if not os.path.exists(labels_path):
                print(f"Warning: Label file {labels_path} not found. Skipping.")
                continue

            labels = pd.read_csv(labels_path)
            
            # 修正！先在原始的 labels DataFrame 上创建 'path1' 列
            labels['path1'] = 'images/' + game + '/' + clip + '/' + labels['file name']
            
            if len(labels) < 3:
                print(f"Warning: Clip {clip} has fewer than 3 frames. Skipping.")
                continue
            
            labels_target = labels[2:].copy()
            
            if labels_target.empty:
                continue

            # 修正！直接从原始的 labels DataFrame 上取 'path1' 列，然后进行切片操作
            labels_target.loc[:, 'path2'] = list(labels['path1'][1:-1])
            labels_target.loc[:, 'path3'] = list(labels['path1'][:-2])
            labels_target.loc[:, 'gt_path'] = 'gts/' + game + '/' + clip + '/' + labels_target['file name']
            
            dfs_to_concat.append(labels_target)
